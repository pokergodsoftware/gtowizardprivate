...
export const ComboDetail: React.FC<ComboDetailProps> = ({ selectedHand, currentNode, bigBlind, settings, displayMode }) => {
  ...
  return (
    <div className="flex-1 bg-[#1e2227] rounded-lg p-4 flex flex-col space-y-4 overflow-y-auto">
      <div>
      ...
      <div className="space-y-2">
        <h4 className="font-semibold text-gray-300 text-sm uppercase tracking-wide">Ações</h4>
        {actionDetails.map((detail, index) => detail && (
          <div key={index} className="bg-[#2d3238] p-3 rounded-lg">
            <div className="flex justify-between items-center mb-1">
                <span className={`font-bold text-base ${detail.color.replace('bg-', 'text-')}`}>{detail.actionName}</span>
                <span className="text-xl font-bold text-white">{(detail.frequency * 100).toFixed(1)}%</span>
            </div>
      ...
      {/* Combos List */}
       <div className="flex-grow overflow-y-auto">
        <h4 className="font-semibold text-gray-300 mb-2 sticky top-0 bg-[#1e2227] py-1 text-sm uppercase tracking-wide">Combos ({allCombos.length})</h4>
        <div className="grid grid-cols-4 gap-x-2 gap-y-1 text-center">
            {allCombos.map(combo => {
                ...
                const s2 = combo[3];
                return (
                    <div key={combo} className="font-mono text-sm text-gray-300 bg-[#2d3238] rounded-md p-1.5 hover:bg-[#353a42] transition-colors">
                        <span>{r1}</span>
